...
const ModalOrderML = ({
    ...
    if (order && order.shipping.id) {
      try {
        const response = await api.get(
          `https://api.helpdesk-maraba.cloud/api-ml/shipments/${order.shipping.id}`,
        );
  ...
  const getValue = <K extends keyof OrderMl>(key: K) => {
    ...
    if (key === "date_created") {
      return editedOrder?.date_created
        ? format(new Date(editedOrder.date_created), "dd/MM/yyyy HH:mm:ss", {
            locale: ptBR,
          })
        : "";
    }
      ...
      <div className="relative m-6 md:m-16 bg-white max-w-lg w-full">
        ...
        <div className="purple_border p-8 border border-black">
          <h3 className="text-lg font-bold mb-4">
            Detalhes do Pedido ID: {order.id}
          </h3>
              ...
              <div key={key} className="mb-4">
                ...
                {type === "select" ? (
                  <select
                    ...
                    onChange={(e) =>
                      handleChange(
                        key as keyof OrderMl,
                        e.target.value as OrderMl[typeof key],
                      )
                    }
                  ...
                  <input
                    type={type}
                    value={
                      type === "text" ? getValue(key as keyof OrderMl) : ""
                    }
                    onChange={(e) =>
                      handleChange(
                        key as keyof OrderMl,
