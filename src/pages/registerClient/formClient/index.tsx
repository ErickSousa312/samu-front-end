...
const onSubmit = async (data: OrderFormData) => {
  try {
    addToast({ type: 'loading', message: 'Aguarde, estamos criando um cliente...' });
    const dataWithRole = { ...data, role: 'customer' };

    await api.post(`/signUp`, dataWithRole);
    addToast({ type: 'success', message: 'Cliente criado com sucesso!' });
  } catch (err) {
    addToast({ type: 'error', message: 'Falha ao criar cliente.' });
